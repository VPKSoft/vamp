...
using CefSharp.WinForms; // (C): https://github.com/cefsharp/CefSharp/, Modified BSD (3-clause BSD license)
...
{
    /// <summary>
    /// A full screen web browser window (Chromium based).
    /// </summary>
    public partial class FormWebBrowserChromium : DBLangEngineWinforms
    {
        ...
        public FormWebBrowserChromium(string URL, bool homeButton = true)
        {
            ...
            {
                Dock = DockStyle.Fill, // dock the browser to fill the form..                         
            };

            Controls.Add(browser); // add the ChromiumWebBrowser to the form's controls

            browser.AddressChanged += Browser_AddressChanged; // Assign some browser events..
            browser.LoadError += Browser_LoadError;
            browser.LoadingStateChanged += Browser_LoadingStateChanged; // END: Assign some browser events..

            m_GlobalHook = Hook.GlobalEvents(); // The ChromiumWebBrowser prevents the Form from getting mouse signals, so we add this (Gma.System.MouseKeyHook) event handler..
        ...
        #region ChromiumWebBrowserEvents
        ...
        private void WebBrowserForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                browser.AddressChanged -= Browser_AddressChanged; // release the event handlers..
                browser.LoadError -= Browser_LoadError;
...
//                SetButtonStates(); // sets the button states to enabled / disabled depending on the browser component's state..
        ...
        {
            btnForward.Enabled = browser.GetBrowser().CanGoForward; // if forward navigation is possible in the browser..
            btnBack.Enabled = browser.GetBrowser().CanGoBack; // if backward navigation is possible in the browser..
